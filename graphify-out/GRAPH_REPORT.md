# Graph Report - molty5  (2026-05-08)

## Corpus Check
- 39 files · ~51,769 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 670 nodes · 992 edges · 73 communities detected
- Extraction: 86% EXTRACTED · 14% INFERRED · 0% AMBIGUOUS · INFERRED: 137 edges (avg confidence: 0.62)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `edc625ba`
...
Cohesion: 0.05
Nodes (41): Strategy Brain Module, ActionSender, Tracks cooldown state and builds action envelopes., Update state from action_result payload.         Per actions.md: canAct and cool, Update state from can_act_changed server push., Can we send a Group 1 (cooldown) action?, Build action envelope per actions.md spec.         Truncates thought fields to s, Per actions.md: requires megaphone item or broadcast_station facility.         S (+33 more)
...
Cohesion: 0.16
Nodes (11): APIError, Join a game based on room selection., Join a game based on room selection., Join a game based on room selection., Join a game based on room selection., Exception, join_free_game(), Free game join via matchmaking queue. POST /join (Long Poll ~15s) → assigned → o (+3 more)

### Community 13 - "Community 13"
Cohesion: 0.14
Nodes (4): AgentMemory, Read/write molty-royale-context.json with overall + temp sections.      New stru, Read/write molty-royale-context.json with overall + temp sections., Recalculate threat_rating for all opponents based on stats.

### Community 14 - "Community 14"
Cohesion: 0.17
Nodes (11): Setup pipeline: wallet → whitelist → identity. Respects config flags., Setup pipeline: wallet → whitelist → identity. Respects config flags., Setup pipeline: wallet → whitelist → identity. Respects config flags., Setup pipeline: wallet → whitelist → identity. Respects config flags., ensure_identity(), Ensure ERC-8004 identity is registered.     Returns True if identity is set. Nev, ensure_molty_wallet(), MoltyRoyale Wallet (SC Wallet) setup — POST /create/wallet. Handles CONFLICT, WA (+3 more)
...
Cohesion: 0.17
Nodes (8): Heartbeat loop — main orchestration per heartbeat.md. State machine: setup → joi, Whitelist Setup Module, Logger Utility, EIP-712 Signer Module, Identity Contract Module, Action envelope builder + cooldown state tracker. Builds action messages per act, Paid game join — EIP-712 sign → POST /games/{id}/join-paid. Per paid-games.md: c, ERC-8004 Identity registration — on-chain register() + POST /api/identity. Never

### Community 19 - "Community 19"
Cohesion: 0.22
Nodes (9): calc_damage(), decide_action(), Damage formula per combat-items.md + game-systems.md weather penalty.     Base:, Resolve a connectedRegions entry to a full region object.     Per v1.5.2 gotcha, Resolve a connectedRegions entry to a full region object.     Per v1.5.2 gotchas, Main decision engine. Returns action dict or None (wait).      Priority chain, Main decision engine. Returns action dict or None (wait).      Priority chain pe, Damage formula per combat-items.md + game-systems.md weather penalty.     Base: (+1 more)
...
## Knowledge Gaps
- **296 isolated node(s):** `Molty Royale AI Agent Bot`, `State router — determines agent state from GET /accounts/me response. Routes per`, `Analyze /accounts/me response → return (state, context).     Context contains re`, `Exception`, `Async REST API client for Molty Royale. All endpoints from api-summary.md with r` (+291 more)
...
- **Why does `Logger Utility` connect `Community 18` to `Community 0`, `Community 1`, `Community 2`, `Community 7`, `Community 8`, `Community 9`, `Community 11`, `Community 12`, `Community 14`, `Community 15`, `Community 16`, `Community 17`, `Community 20`, `Community 23`, `Community 25`, `Community 29`, `Community 30`, `Community 32`, `Community 34`, `Community 46`, `Community 47`, `Community 49`?**
  _High betweenness centrality (0.384) - this node is a cross-community bridge._
- **Why does `Heartbeat` connect `Community 8` to `Community 2`, `Community 12`, `Community 14`, `Community 15`?**
  _High betweenness centrality (0.116) - this node is a cross-community bridge._
- **Why does `MoltyAPI` connect `Community 4` to `Community 7`, `Community 12`, `Community 14`, `Community 15`, `Community 49`, `Community 18`, `Community 30`?**
